import { View, Text, SafeAreaView, Dimensions, ScrollView, TouchableOpacity, StyleSheet, TouchableWithoutFeedback, Alert } from 'react-native'
...
// import Eclipse from '../../../EclipseLP.js'
export default function VisualStoriesSID({ navigation }) {
  ...
  <canvas id="canvas" width=${width * 0.99} height=${height * 0.946} style="border:1px solid; border-color:black; "></canvas>
  ...
  var message = function(event) {
    ...
    if(msgData === '${actions.getPoints}'){
      ...
      // let a = JSON.stringify(imgData);
      let canvasURL = canvas.toDataURL("image/png", 1)
      window.ReactNativeWebView.postMessage(canvasURL);
      a = undefined
    }
  ...
  const stickList = ['https://i.gifer.com/2iiG.gif', 'https://compote.slate.com/images/697b023b-64a5-49a0-8059-27b963453fb1.gif']
  ...
  const onMessage = (data) => {
    ...
    // let b = JSON.parse(a)
    // setPointsData(b);
    // const info = Object.keys(a).reduce(function (info, key) {
    //   if (dict[key] !== 0) info[key] = a[key]
    //   return info
    // }, {})
    // a = info
    let arr = anim.current
    navigation.navigate('Preview', { a, arr })
  }
  ...
  // console.log(image)
  const updateSticker = (sticker, index) => {
    var tempAnim = anim.current
    tempAnim = tempAnim.map((anim) => {
      if (anim.id === index) {
        anim = sticker
      }
      return anim
    })
    anim.current = tempAnim
  }
  ...
  const addSticker = (uri) => {
    ...
    setModal(false)
    console.log('sticker added   arrLen:' + anim.current.length)
  }

  const replaceSticker = (stickerURI) => {
    var tempAnim = anim.current
    var sticker = tempAnim.filter(sticker => sticker.id === repId)[0]
    const checkSticker = (element) => { element.id === sticker.id }
    const index = tempAnim.findIndex(checkSticker)
    console.log('old:' + JSON.stringify(sticker))
    sticker['stickerURI'] = stickerURI
    console.log('new:' + JSON.stringify(sticker))
    tempAnim[index] = sticker
    anim.current = tempAnim
    setRepModal(false)
  }

  const delSticker = (index) => {
    console.log('deleting at index:' + index)
    console.log(anim.current)
    var tempAnim = [...anim.current]
    var newAnim = []
    newAnim = tempAnim.filter(sticker => sticker.id != index)
    anim.current = newAnim
  }

  const checkPress = (index) => {
  ...
  return (
    <SafeAreaView style={{ height, width, backgroundColor: 'white', zIndex: 1 }} >
      <WebView
        bounces={false}
        scrollEnabled={false}
        ...
        onMessage={onMessage}
        style={{ height: height, width: cust * 1.0 }}
        originWhitelist={['*']}
      >
        {anim.current.map((sticker) => {
          console.log(anim.current)
          console.log(sticker)
          return (
            <Sticker uri={anim.current} key={sticker.id} id={sticker.id} updateSticker={updateSticker} delSticker={checkPress} />
          )
        })}
      </WebView>

      {/* <View style={{flexDirection:'row',paddingBottom:'1%',backgroundColor:'white', alignSelf: 'center'}}>
        ...
        <TouchableOpacity style={styles.bottomButton} onPress={() => setModal(true)}>
          <Text>Animation</Text>
        </TouchableOpacity>
      </View> */}
      ...
      <ReactNativeModal isVisible={repModal} onBackdropPress={() => setRepModal(false)}>
        <View style={styles.modal}>
          {stickList.map((stickerURI, index) => {
            return (
              <TouchableWithoutFeedback style={styles.pinchableImage} onPress={() => replaceSticker(stickerURI)} key={index}>
                <Animated.Image
                  style={[
      ...
      <ReactNativeModal isVisible={modal} onBackdropPress={() => setModal(false)}>
        <View style={styles.modal}>
          {stickList.map((stickerURI, index) => {
            return (
              <TouchableWithoutFeedback style={styles.pinchableImage} onPress={() => addSticker(stickerURI)} key={index}>
                <Animated.Image
                  style={[
                    styles.pinchableImage,
                    {
